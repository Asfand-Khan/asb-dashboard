...
const FormLayout = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await toast.promise(axios.delete(`/api/service/${id}`), {
        pending: "Deleting Service",
        success: "Blog service successfully",
            ...
            columns={[
              {
                cell: (row: Blog) => (
                  <span className="flex items-center justify-center gap-2">
                    ...
                    <a
                      target="_blank"
                      href={`https://aussie-steel-beams.vercel.app/services/${row.slug}`}
                      className="cursor-pointer"
                    >
                      <Image
